package inp;
import pac.*;
import pac.day.DayData;
import pac.day.DayPac;
import pac.inter.Storable;
import tol.BytesTo;

import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;

public class DayIn implements Inputer {
    static final Class<?> pactype= DayPac.class;
    String filelocation;
    File file=null;
    public DayIn(String location){
        filelocation=location;
    }
    public DayIn(File infile){file=infile;}
    public Class<?> getPacType(){ return pactype; }
    public Storable getPackage(){
        if(file==null) {
            if (filelocation == null) {
                try {
                    throw new Exception("inputer has not been initialized!");
                }catch (Exception e){
                    System.out.println(e);
                    e.printStackTrace();
                }
                return null;
            }
            file = new File(filelocation);
        }
        FileInputStream fis=null;
        try {
            fis = new FileInputStream(file);
        }catch (IOException e){
            System.out.println(e);
        }

        String filename=file.getName();
        String se=filename.substring(0,2);
        int sn=Integer.parseInt(filename.substring(2,8));
        DayPac pac=new DayPac(se,sn);

        int probe;
        int counter=0;
        try {
            probe=fis.read();
            while (probe!=-1) {
                fis.skip(-1);
                //System.out.println(counter++);
                byte[][]bytes=new byte[8][4];
                for(int i=0;i<8;i++){
                    fis.read(bytes[i]);
                }

                DayData day=new DayData(BytesTo.Bytes2int(bytes[0]),BytesTo.Bytes2int(bytes[1]),
                        BytesTo.Bytes2int(bytes[2]),BytesTo.Bytes2int(bytes[3]),
                        BytesTo.Bytes2int(bytes[4]),BytesTo.Bytes2float(bytes[5]),
                        BytesTo.Bytes2int(bytes[6]));
                pac.newDay(day);
                probe=fis.read();
            }
        }catch(IOException e){
            System.out.println(e);
        }
        filelocation=null;
        file=null;
        return pac;
    }
}